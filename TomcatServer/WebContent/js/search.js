/**
 * Tag info fra Search, lav td og tr elementer fra dette.
 */
window.onload = function() {
	sendRequest("GET", "rest/indexer/search2", null, function(itemsText) {
	...
	addEventListener(updateButton, "click", function() {
		sendRequest("GET", "rest/indexer/search2?query=" + document.getElementById("query").value, null, function(itemsText) {
			var items = JSON.parse(itemsText);
			addItemsToTable(items);
		});
	});
